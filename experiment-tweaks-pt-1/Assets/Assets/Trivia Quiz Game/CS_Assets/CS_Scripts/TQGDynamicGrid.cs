...
namespace TriviaQuizGame
{
	...
	public class TQGDynamicGrid:MonoBehaviour
	{
		...
		void Update()
		{
			...
			//{
				// Check if the screen aspect reatio is vertical or horizontal, and set the grid cell size accordingly
				if ( Screen.width/Screen.height >= 1 )    GetComponent<GridLayoutGroup>().cellSize = horizontalCellSize;
				else    GetComponent<GridLayoutGroup>().cellSize = verticalCellSize;
